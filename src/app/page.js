          ...
          list.map((data, i) => {
            return (
              <>
                ...
                <button className="btn3" onClick={()=>editActivity(i)}>edit Activity</button>
                </p>
              </>  ); 
            })}
        {list.length == 0 && <h2>No Task Available</h2>}
        {list.length > 1 && ( <button className="removeallbtn" onClick={removeAll}> Remove All</button> )}
      </div>
    </>
  );
};
